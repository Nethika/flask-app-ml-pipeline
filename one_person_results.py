...
## Ethnicity model
model_ethnicity = load_model("models/ethnicity.model")
model_ethnicity._make_predict_function()
graph_eth = tf.get_default_graph()
print("INFO..............Ethnicity Model Loaded")
...
class Handler(FileSystemEventHandler):
    ...
    def on_any_event(event):
        ...
        if event.is_directory:
            ...
            out_mask = visualize.display_instances(m_image, r['rois'], r['masks'], r['class_ids'], 
                                        class_names, r['scores'])

            out_path = "yolo3_result/" + datetime_name + "_mask.png"
            skimage.io.imsave(out_path,out_mask)

            ### CALL YOLO 3
            with graph_yolo3.as_default():
                out_image,objects= calc_objects_hug(yolo,im_pil)
            
            
            output_image = "yolo3_result/" + datetime_name+ ".png"
            out_image.save(output_image, quality=100)

            #pilImg=Image.fromarray(skimage.util.img_as_ubyte(img))
...
##########################
            ...
            im_path = "result_images/" + datetime_name + "_before_drawing.png"
            ...
            if len(detected) > 0:
                ...
                for i, d in enumerate(detected):
                    ...
                    print("INFO..............Calculating Ethnicity.....")
                    ...
                    with graph_eth.as_default():
                        ...
                        c_l_all = model_ethnicity.predict_classes(im, verbose=0)
                        percentage = model_ethnicity.predict(im, verbose=0)
                    
                    #print("Face #: ",i)
                    print ("Ethnicity:",classes[class_labels])
                    #print ("class_labels",class_labels)
                    #print ("class_labels all", c_l_all)
                    #print ("percentages", percentage)
                    #for cl,per in zip(classes, percentage[0]):
                    #    print (cl, ":", per)
                ...
                for i in range(len(features)): #faces in new image
                    ...
                    if prof_match:
                        ...
                        real_ethnicity = prof_data[indx]['ethnicity']
                    ...
                    if not prof_macthed_wrote:
                        face_gender = "F" if predicted_genders[i][0] > 0.5 else "M"
                        face_age = int(predicted_ages[i])
                        face_emotion = emotion_class[i]

                        result_dict = { "resut_image_path": output_image, 
                                        "calculated_age": face_age, 
                                        "calculated_gender": face_gender,
                                        "calculated_emotion": face_emotion,
